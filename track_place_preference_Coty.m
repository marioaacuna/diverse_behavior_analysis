...
% place aversion chamber for one animal. . Additionally, you can select if
...
% in a .mat file.
% this is for Coty's experiments.$
% These experiments are done in the behavior room, outisde the soundproof box.Therefore,
% the camera needed to be on a tripod. Light conditions and position of the camera can change.
% For some experiments, the min area needs to be modified,  or the cropping, to avoid having too much
% of black background.
% Overall it works well. Even if an experimenter's hand is below the animal.
% cd('M:\Mario\Liselot')
...
hold on
plt = plot(w-data_1(:,1), l-data_1(:,2), 'ro', 'MarkerSize', 1);
% figure, plot(fliplr([w-data_1(:,1), l-data_1(:,2)]))
%%
